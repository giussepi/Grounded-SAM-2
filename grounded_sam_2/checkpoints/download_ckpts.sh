...
t_file="sam2.1_hiera_tiny.pt"
sam2p1_hiera_t_url="${SAM2p1_BASE_URL}/${t_file}"
s_file="sam2.1_hiera_small.pt"
sam2p1_hiera_s_url="${SAM2p1_BASE_URL}/${s_file}"
b_file="sam2.1_hiera_base_plus.pt"
sam2p1_hiera_b_plus_url="${SAM2p1_BASE_URL}/${b_file}"
l_file="sam2.1_hiera_large.pt"
sam2p1_hiera_l_url="${SAM2p1_BASE_URL}/${l_file}"

# SAM 2.1 checkpoints
if ! [ -f ./$t_file ]; then
    echo "Downloading ${t_file} checkpoint..."
    $CMD $sam2p1_hiera_t_url || { echo "Failed to download checkpoint from $sam2p1_hiera_t_url"; exit 1; }
else
    echo "${t_file} already exists; not retrieving"
fi

if ! [ -f ./$s_file ]; then
    echo "Downloading ${s_file} checkpoint..."
    $CMD $sam2p1_hiera_s_url || { echo "Failed to download checkpoint from $sam2p1_hiera_s_url"; exit 1; }
else
    echo "${s_file} already exists; not retrieving"
fi

if ! [ -f ./$b_file ]; then
    echo "Downloading ${b_file} checkpoint..."
    $CMD $sam2p1_hiera_b_plus_url || { echo "Failed to download checkpoint from $sam2p1_hiera_b_plus_url"; exit 1; }
else
    echo "${b_file} already exists; not retrieving"
fi

if ! [ -f ./$l_file ]; then
    echo "Downloading ${l_file} checkpoint..."
    $CMD $sam2p1_hiera_l_url || { echo "Failed to download checkpoint from $sam2p1_hiera_l_url"; exit 1; }
else
    echo "${l_file} already exists; not retrieving"
fi
